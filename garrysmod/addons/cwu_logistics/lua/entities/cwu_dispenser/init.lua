...
if SERVER then
    util.AddNetworkString("CWU_OpenMinigame_Ration")
    util.AddNetworkString("CWU_FinishMinigame_Ration")
end
...
function ENT:Initialize()
    if SERVER then
        ...
        timer.Create("DispenserAutoRefill_" .. self:EntIndex(), 600, 0, function()
            ...
            if not hasSupplier and self:GetRations() < 5 then
                self:SetRations(self:GetRations() + 1)
            end
        end)
    end
end
...
if SERVER then
    function ENT:SpawnFunction(ply, tr)
        ...
        local ent = ents.Create("cwu_dispenser")
...
function ENT:Use(activator, caller)
    ...
    if activator:Team() == TEAM_SUPPLIER and activator:HasWeapon("weapon_cwu_box") then
        if self:GetRations() >= 5 then
            DarkRP.notify(activator, 1, 5, "Ваші дії викликають питання. Автовидавач вже повний!")
            return
        end
        
        -- Відкриваємо мінігру
        net.Start("CWU_OpenMinigame_Ration")
        net.WriteEntity(self)
        net.WriteInt(self:GetRations(), 32)
        net.Send(activator)
        return
    end

    -- Логіка для звичайних гравців (покупка)
    if self:GetRations() <= 0 then
        DarkRP.notify(activator, 1, 4, "Автовидавач порожній, повідомте ГСП!")
        return
    end
    ...
    if timeLeft > 0 then
        DarkRP.notify(activator, 1, 4, "Помилка. Наступний раціон буде доступний через " .. math.ceil(timeLeft) .. " секунд.")
        return
    end

    if activator:canAfford(0) then
        activator:addMoney(0)
        DarkRP.notify(activator, 0, 4, "Ви придбали раціон.")
        
        self:SetRations(self:GetRations() - 1)
        activator.RationCooldowns[self] = CurTime() + 300 -- 5 хвилин кд
        
        -- Спавн раціону
        local ration = ents.Create("cwu_ration_item")
...
net.Receive("CWU_FinishMinigame_Ration", function(len, ply)
    local dispenser = net.ReadEntity()
    if not IsValid(dispenser) or dispenser:GetClass() ~= "cwu_dispenser" then return end
    ...
    local prevRations = dispenser:GetRations()
    ...
    ply:addMoney(reward)
    DarkRP.notify(ply, 0, 5, "Ви поповнили автовидавач і отримали " .. reward .. " токенів!")
    
    dispenser:SetRations(5)
